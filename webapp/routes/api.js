...
import * as subscriptionStore from '../lib/subscriptionStore.js';
import { requireCourseCreation, requireExamCreation, requirePdfExport, trackUsage } from '../lib/subscriptionMiddleware.js';
...
apiRouter.get('/subscription/plans', async (req, res) => {
  try {
    const plans = await subscriptionStore.getSubscriptionPlans();
...
apiRouter.get('/subscription/me', authenticate, async (req, res) => {
  try {
    const subscription = await subscriptionStore.getUserSubscription(req.user.id);
    const limits = await subscriptionStore.getUserPlanLimits(req.user.id);
    const now = new Date();
    const yearMonth = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    const usage = await subscriptionStore.getMonthlyUsage(req.user.id, yearMonth);
    
    res.json({ 
      success: true, 
      subscription, 
      limits, 
      usage: usage || { courses_created: 0, exams_created: 0, questions_generated: 0 }
    });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
...
apiRouter.post('/subscription/subscribe', authenticate, async (req, res) => {
  try {
    ...
    const plan = await subscriptionStore.getSubscriptionPlan(planId);
    if (!plan) {
      return res.status(400).json({ success: false, error: 'Invalid plan' });
    }
    
    if (plan.price > 0) {
      return res.status(400).json({ success: false, error: 'Payment integration required for paid plans' });
    }
    
    const subscription = await subscriptionStore.createUserSubscription(req.user.id, planId);
...
apiRouter.post('/subscription/cancel', authenticate, async (req, res) => {
  try {
    const subscription = await subscriptionStore.cancelUserSubscription(req.user.id);
    res.json({ success: true, subscription });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
