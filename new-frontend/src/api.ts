...
export interface SubscriptionPlan {
  ...
  max_questions_per_exam: number | null;
  features: string[];
  watermark: boolean;
  pdf_export: boolean;
  lms_export: boolean;
  api_access: boolean;
  mass_generation: boolean;
  active: boolean;
}
...
export async function getSubscriptionPlans(): Promise<{ success: boolean; plans: SubscriptionPlan[] }> {
  return request('/subscription/plans');
}

export async function getSubscriptionMe(): Promise<{ success: boolean; subscription: UserSubscription | null; limits: PlanLimits; usage: MonthlyUsage }> {
  return request('/subscription/me');
}

export async function postSubscriptionSubscribe(planId: string): Promise<{ success: boolean; subscription: UserSubscription }> {
  return request('/subscription/subscribe', { method: 'POST', body: { planId } });
}

export async function postSubscriptionCancel(): Promise<{ success: boolean; subscription: UserSubscription }> {
