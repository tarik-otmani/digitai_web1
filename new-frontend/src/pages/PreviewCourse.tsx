...
export default function PreviewCourse() {
  ...
  const handleExportPdf = async () => {
    ...
    setExporting(true);
    try {
      const url = getCourseExportPdfUrl(id);
      const res = await fetch(url);
      if (!res.ok) throw new Error(await res.text());
      const blob = await res.blob();
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = `${(course?.topic || 'course').replace(/[^a-zA-Z0-9-_]/g, '_')}.pdf`;
      a.click();
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={exporting}
            className="px-4 py-2 bg-indigo-600 text-white rounded-lg font-medium flex items-center gap-2 hover:bg-indigo-700 disabled:opacity-60"
          >
            {exporting ? (
